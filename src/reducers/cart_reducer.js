...
const cart_reducer = (state, action) => {
  switch (action.type) {
    ...
    case REMOVE_CART_ITEM:
      const { id: REMOVE_CART_ITEM_ID } = action.payload;
      const newCart = state.cart.filter((c) => c.id !== REMOVE_CART_ITEM_ID);
    ...
    case COUNT_CART_TOTALS:
      const { total_amount, total_items } = state.cart.reduce(
        (total, cartItem) => {
          ...
          total.total_items = total.total_items + cartItem.amount;
          return total;
        },
        {
          total_items: 0,
